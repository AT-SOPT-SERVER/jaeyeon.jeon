...
public class PostServiceValidator {
    ...
    public void titleNotBlankValidate(String title) {
        if (title == null || title.trim().isEmpty()) {
            throw new PostHandler(CommonErrorStatus._POST_TITLE_NOT_NULL);
        }
    }

    public void contentNotBlankValidate(String content) {
        if (content == null || content.trim().isEmpty()) {
            throw new PostHandler(CommonErrorStatus._POST_CONTENT_NOT_NULL);
        }
    }
    ...
    public void titleLengthValidate(String title) {
        ...
        if (length > 30){
            throw new PostHandler(CommonErrorStatus._POST_TITLE_LENGTH);
        }
    }
    ...
    public void titleDuplicate(String title) {
        ...
        if (postRepository.existsByTitle(title)){
            throw new PostHandler(CommonErrorStatus._POST_TITLE_DUPLICATE);
        }
    }
    ...
    public void contentLengthValidate(String content) {
        ...
        if (length > 1000){
            throw new PostHandler(CommonErrorStatus._POST_CONTENT_LENGTH);
        }
    }
